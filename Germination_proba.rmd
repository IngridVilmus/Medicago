...
germ_rbimod5<-glmer(cbind(GerminationSurvival,GerminationDeath) ~ zone + (1|Pop_geno_mother) + (1|Pop_geno) + (1|zone:Pop_geno)+ (1|quadrat)+ (1|location) ,data=don_bimod,family=binomial(link=binlink))


summary(germ_rbimod5)

# check for convergence warnings given by glmer
relgrad <- with(germ_rbimod5@optinfo$derivs,solve(Hessian,gradient))
max(abs(relgrad))

germ_rbimod5<-glmer(cbind(GerminationSurvival,GerminationDeath) ~ zone + (1|Pop_geno/Pop_geno_mother) + (1|zone:Pop_geno)+ (1|quadrat)+ (1|location) ,data=don_bimod,family=binomial(link=binlink))
logLik(germ_rbimod5)
